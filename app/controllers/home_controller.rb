class HomeController < ApplicationController
  ...
  def index
    @cards = Card.virtual.vslip_present.search(params[:search]).expansion(params[:expansion]).side(params[:side])
    @cards = params[:legacy] ? @cards.legacy : @cards.legal
    @cards = @cards.paginate(:per_page => 20, :page => params[:page])
